...
const SocialShare = ({ title, url, excerpt = '' }) => {
  ...
  return (
    <div className="relative">
      ...
      {showDropdown && (
        <div className="absolute top-full mt-2 right-0 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 min-w-48 z-10 transition-colors duration-200">
          {socialLinks.map(({ name, icon: Icon, url, color }) => (
            <button
              key={name}
              onClick={() => handleShare(url)}
              className={`w-full flex items-center space-x-3 px-4 py-2 text-left text-gray-700 dark:text-gray-300 transition-colors duration-200 ${color}`}
            >
